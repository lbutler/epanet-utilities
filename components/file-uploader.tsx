...
export function FileUploader({ onFileLoaded }: FileUploaderProps) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Upload EPANET File</h2>
      ...
      {!file ? (
        ...
        >
          <div className="flex flex-col items-center justify-center space-y-3">
            ...
            <div>
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Drag and drop your INP file here</p>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">or click to browse files</p>
            </div>
            <input type="file" accept=".inp" className="hidden" id="file-upload" onChange={handleFileChange} />
            <label
              htmlFor="file-upload"
